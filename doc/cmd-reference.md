...
### Beam Generators
 ...
 - [ParticleGun](./PrimGen/ParticleGun.md)
 - [General Particle Source, GPS](./PrimGen/GPS.md)
 - [BeamGun](./PrimGen/BeamGun.md)
 - [ScanBeam](./PrimGen/ScanBeam.md)
 - [FocusGun](./PrimGen/FocusGun.md)
 - [FocusGunEdist](./PrimGen/FocusGunEdist.md)
...
| 値表記 | 説明 |
|:---|:---|
| dx:d, dy:d, dz:d, unit:s | 各辺の長さの半値と単位 |

#### 水ファントムの登録
```
 /Dynamic/Module/WaterPhantom/register  {mname:s}
```
| 値表記 | 説明 |
|:---|:---|
| mname:s | モジュール名。デフォルトは`WaterPhantom`.|
...
### シミュレーション空間のデバック出力
```
/My/DetConstruction/verbose {lvl:i}
```
| 値表記 | 説明 |
|:---|:---|
| lvl:i | verbose level.|
...
### ファイル出力後もG4AnalysisManagerが結果を保持
Geant4のVisualizationが提供するplotter機能を使って結果を表示するために、Run終了後もヒストグラム情報を保持することができる。
継続してRunを実行すると、スコア結果は追加されていく。
```
/My/runaction/analysis/keep {bkeep:b}
```
| 値表記 | 説明 |
|:---|:---|
| bkeep:b | true=ヒストグラムを保持。false=ヒストグラムはRunごとにリセットされる。 |
...
### ヒストグラム H1,H2,H3,P1,P2 (G4AnalysisManager)
G4AnalysisManagerのコマンドを使ってヒストグラムを宣言します。

#### ヒストグラム宣言
```
/analysis/h1/create {htname:s} {nbinx:i} {xmin:d} {xmax:d} {xunit:s} {xfcn:s} {xbinScheme:s}
/analysis/h2/create {htname:s} {nbinx:i} {xmin:d} {xmax:d} {xunit:s} {xfcn:s} {xbinScheme:s}
		               {nbiny:i} {ymin:d} {ymax:d} {yunit:s} {yfcn:s} {ybinScheme:s}
/analysis/h3/create {htname:s} {nbinx:i} {xmin:d} {xmax:d} {xunit:s} {xfcn:s} {xbinScheme:s}
		               {nbiny:i} {ymin:d} {ymax:d} {yunit:s} {yfcn:s} {ybinScheme:s}
		               {nbinz:i} {zmin:d} {zmax:d} {zunit:s} {zfcn:s} {zbinScheme:s}
/analysis/p1/create {htname:s} {nbinx:i} {xmin:d} {xmax:d} {xunit:s} {xfcn:s} {xbinScheme:s}
		               {nbiny:i} {ymin:d} {ymax:d} {yunit:s} {yfcn:s} {ybinScheme:s}
/analysis/p2/create {htname:s} {nbinx:i} {xmin:d} {xmax:d} {xunit:s} {xfcn:s} {xbinScheme:s}
		               {nbiny:i} {ymin:d} {ymax:d} {yunit:s} {yfcn:s} {ybinScheme:s}
		               {nbinz:i} {zmin:d} {zmax:d} {zunit:s} {zfcn:s} {zbinScheme:s}
```
| 値表記 | 説明 |
|:---|:---|
| htname:s |  histogram 名 |
| nbin:i |  Number of bins |
| min:d |  Minimum edge  |
| max:d |  Maximum edge  |
| unit:s | Unit of the axis  |
| Fcn:s | Function applied for the filling value. log, log10, exp, none  |
| bunScheme:s | Binning type of the axies. linear, log |


#### ヒストグラムとヒットコレクションとの対応付
G4AnalysisManagerのコマンドを使って作成した、H1, H2, H3, P1, P2のヒストグラムとヒットコレクションの対応付けを行います。
対応付られたヒットコレクションから指定されたスコア量が取り出されて、ヒストグラムにフィルされる。
```
/My/runaction/h1/link  {htmame:s}  {xpar:s} {wandunit:s} {fullcollection:s}
/My/runaction/h2/link  {htmame:s}  {xpar:s} {ypar:s} {wandunit:s} {fullcollection:s}
/My/runaction/h3/link  {htmame:s}  {xpar:s} {ypar:s} {zpar:s} {wandunit:s} {fullcollection:s}
/My/runaction/p1/link  {htmame:s}  {xpar:s} {ypar:s} {wandunit:s} {fullcollection:s}
/My/runaction/p2/link  {htmame:s}  {xpar:s} {ypar:s} {zpar:s} {wandunit:s} {fullcollection:s}
```
| 値表記 | 説明 |
|:---|:---|
| ntname:s |  Ntuple 名 |
| xpar:s |  x軸となるスコアリングの物理量名(qname) |
| ypar:s |  y軸となるスコアリングの物理量名(qname) |
| zpar:s |  z軸となるスコアリングの物理量名(qname) |
| wandunit:s |  weightの物理量名(qname)と単位(unit)を"/"で区切って指定する（例  de/MeV). weight=1.0の場合には、noneを指定する。 |
| fullcollection:s | SD名/ヒットコレクション名で表記する. Digitizerのコレクションの場合は、コレクション名でも可。|
...
| DigiVolMerger | (Hit->Digi for PET)|  
