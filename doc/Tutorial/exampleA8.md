...
/G4M/ScanBeam/weightFile ./WeightSample.dat
```
同じ強度(つまり、同じ数だけ）で照射した場合、エネルギーが小さいビームは浅い位置で高いピーク線量となり、、エネルギーが大きいビームは深い位置まで届いてピーク線量は低くなります。そのため、エネルギーによらず同じ線量を与えるための照射粒子数を求めるための係数を記述します。この係数は、具体的には同じ照射粒子数の時のピーク線量値に相当します。
...
2     0.     0.  2.
```
1行目はデータ数、２行目以降がスポット座標です。スポット情報は、順にビームID番号(`EIDSample.dat`参照)、X座標\(mm\)、Y座標\(mm\)、付与線量強度です。付与線量強度は、同じ値であればピーク線量が同じになります。
